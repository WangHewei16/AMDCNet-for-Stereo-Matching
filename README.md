...
* `./Figure5.png`: This figure shows a comparison of the other three images processed by the algorithm in this paper and Multi-task Attention Stereo Network (MASNet), Pyramid Stereo Matching Network, Richer Convolutional Features Network. The first column is the original left view image, which are art, baby and Book respectively, the second example is the real parallax image, the third column is the algorithm image in this paper, the fourth column is the result diagram obtained in literature, and the fifth column is the result generated by the algorithm in literature.
